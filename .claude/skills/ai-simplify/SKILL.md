...
3. **Dependency preflight** — verify `.claude/agents/ai-simplify.md` is on disk. STOP and report the exact missing path if absent.
...
**Calls**: the `ai-simplify` agent (`.claude/agents/ai-simplify.md`) via the
...
**See also**:
- `.claude/skills/ai-simplify-sweep/SKILL.md` — scheduled wrapper with draft-PR side effect (different cadence and contract).
