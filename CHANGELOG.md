...
- An install-wizard demo GIF (`docs/demo-install.gif`) showing the real first-run `copilotline install` flow: it wires the statusline, then runs the interactive "Choose quota account" picker listing three fabricated accounts (`octocat` / `monalisa` / `hubot`, all "token ok") and a selection (`monalisa`). Generated PII-free and offline by `docs/fixtures/render-demos.sh` (which seeds the three accounts via `docs/fixtures/seed-install-shell.sh` + `seed-install-harness.mjs` and a demo-only offline `/user` mock, `docs/fixtures/github-user-mock.mjs`, that is never imported by `src/`). Referenced from the README Demo section and documented in `docs/DEMOS.md`.
...
- **Security/privacy:** `copilotline render` no longer reads the host's Copilot account or quota for a contentless or unrelated JSON object on stdin. Previously a successfully-parsed object was always treated as a status payload, so `echo '{}' | copilotline render` (and any object with no recognized key, e.g. `{"zzz":1}`) still detected the real Copilot account and rendered its login + quota — the boundary case left open by the empty/invalid hardening above. Render now treats stdin as a real payload only when the object carries at least one recognized top-level status key; a contentless/unrecognized object takes the same silent neutral `copilotline` placeholder path as empty stdin (no account detection, no `gh`/`sqlite3` spawn, no stderr diagnostic). A minimal real payload (e.g. `{"model":{…}}`) still renders fully. The payload check is now content-aware: a recognized key with an empty value (e.g. `{"model":{}}`, `{"account":{}}`, `{"cwd":""}`, `{"context_window":{}}`) is also treated as contentless and rejected, closing the same leak class, while a meaningful value — including numeric `0` and boolean `false` (e.g. `{"context_window":{"usedPercent":0}}`) — still renders. Header-only payloads now render correctly too: a top-level `request` object (`{"request":{"headers":{…}}}`) and a flat `x-quota-snapshot-*` header bag, which the quota header reader consumes, are recognized and rendered instead of being wrongly rejected to the placeholder.
