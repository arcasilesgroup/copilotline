...
LEGAL_TRANSITIONS: dict[LifecycleState, frozenset[LifecycleState]] = {
    LifecycleState.DRAFT: frozenset(
        {LifecycleState.APPROVED, LifecycleState.ABANDONED}
    ),
    LifecycleState.APPROVED: frozenset(
        {LifecycleState.IN_PROGRESS, LifecycleState.ABANDONED}
    ),
    LifecycleState.IN_PROGRESS: frozenset(
        {LifecycleState.SHIPPED, LifecycleState.ABANDONED}
    ),
...
def reconcile_merged(
    project_root: Path, *, default_branch: str = _DEFAULT_BRANCH
) -> dict:
...
def _dedup_obvious_by_default(
    ...
    try:
        ...
        e_created = json.loads(essentials.read_text(encoding="utf-8")).get(
            "created", ""
        )
    except (OSError, json.JSONDecodeError):
        return None
    ...
    with artifact_lock(project_root, "specs-history"):
        ...
        for path in sorted(specs.glob("*.json")):
            ...
            if target_path.exists() and target_path != path:
                ...
                continue
            renamed.append(
                {"slug": record.slug, "from": path.name, "to": f"{target_id}.json"}
            )
            if dry_run:
                continue
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    _common(mh)
    cs = sub.add_parser(
        "consolidate_shipped", help="Append missing history rows for SHIPPED specs"
    )
    cs.add_argument(
        "--dry-run", action="store_true", help="Preview rows without mutating files"
    )
    _common(cs)
    mi = sub.add_parser(
        "migrate_ids", help="Rename slug sidecars to canonical spec-NNN"
    )
    mi.add_argument(
        "--dry-run", action="store_true", help="Preview renames without mutating files"
    )
    _common(mi)
    rm = sub.add_parser(
        "reconcile_merged",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.cmd == "start_new":
            ...
            print(
                json.dumps(
                    consolidate_shipped(project_root, dry_run=args.dry_run), indent=2
                )
            )
