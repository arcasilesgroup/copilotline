...
_DEFAULT_SURFACE = (
    "claude-code"  # spec-133 D-133-16 fallback when surfaces.enabled is empty
)
...
def _count_skills(root: Path, manifest: dict) -> int:
    skills_rel, _ = _SURFACE_DIRS.get(
        _primary_surface(manifest), _SURFACE_DIRS[_DEFAULT_SURFACE]
    )
...
def _fetch_board_github(
    owner: str, number: int, timeout: float = BOARD_TIMEOUT_S
) -> dict | None:
...
def _board_summary(root: Path, manifest: dict) -> dict:
    ...
    if not isinstance(work_items, dict):
        return {
            "unavailable": True,
            "reason": "work_items missing — run /ai-board discover",
        }
