...
def load_iocs(project_root: Path) -> dict[str, Any]:
    ...
    if cache is not None and sig is not None:
        ...
        if (
            cached_mtime == sig[0]
            and cached_size == sig[1]
            and (now - cached_loaded_at) <= ttl
        ):
    ...
    *,
    ...
    for entry in decisions:
        ...
        if status != "active":
            continue
        risk_category = (
            entry.get("risk_category") or entry.get("riskCategory") or ""
        ).lower()
        if risk_category != "risk-acceptance":
            continue
        expires_at = _parse_decision_timestamp(
...
def _category_patterns(catalog: dict[str, Any], category: str) -> list[tuple[str, str]]:
    ...
    patterns_kind = (
        "regex" if category in ("shell_patterns", "dangerous_commands") else "literal"
    )
...
def evaluate_against_iocs(
    ...
    *,
    ...
    if any_unaccepted:
        names = ", ".join(
            f"{m['category']}:{m['pattern']}" for m in matches if not m["accepted"]
        )
        return {
            "verdict": "deny",
            "matches": matches,
...
def _emit_ioc_outcomes(
    project_root: Path, tool_name: str, result: dict[str, Any]
) -> None:
        ...
        if is_debug_mode():
            ...
            try:
                ...
                with open(debug_log, "a", encoding="utf-8") as f:
                    f.write(
                        f"[{timestamp}] injection scan: tool={tool_name} matches=[{names}]\n"
                    )
            except Exception:
                pass
