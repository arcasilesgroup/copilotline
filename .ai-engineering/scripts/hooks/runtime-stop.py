...
def _active_work_paths(project_root: Path) -> dict[str, str | None]:
    ...
    pointer = read_json(
        project_root / ".ai-engineering" / "specs" / "active-work-plane.json"
    )
    ...
    return {
        "specsDir": str(specs_dir.relative_to(project_root))
        if specs_dir.exists()
        else None,
...
def _ralph_increment_retry(
    ...
    *,
    ...
    exhausted = retries >= _RALPH_MAX_RETRIES
    reason = (
        "convergence_failed: " + "; ".join(failures[:3])
        if failures
...
def _safe_transcript_aggregate(
...
def _safe_write_checkpoint(
    project_root: Path, payload: dict, *, writer=write_json
) -> str | None:
...
def main() -> None:
    ...
    history = recent_tool_history(
        project_root, session_id=session_id, limit=LOOP_WINDOW * 4
    )
...
if __name__ == "__main__":
    run_hook_safe(
        main, component="hook.runtime-stop", hook_kind="stop", script_path=__file__
    )
