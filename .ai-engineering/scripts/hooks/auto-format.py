...
def _detect_js_formatter(project_root: Path) -> str | None:
    ...
    if (project_root / "biome.json").exists() or (
...
def _format_js_ts(file_path: str, project_root: Path) -> None:
    ...
    if formatter == "biome":
        ...
        if local_bin:
            _run_formatter(
                [local_bin, "format", "--write", file_path], cwd=str(project_root)
            )
        else:
            _run_formatter(
                ["npx", "biome", "format", "--write", file_path], cwd=str(project_root)
            )
    else:
        ...
        if local_bin:
            ...
            _run_formatter(
                ["npx", "prettier", "--write", file_path], cwd=str(project_root)
            )
