...
def verify_hook_integrity(
    ...
    if expected is None:
        if _resolve_mode() == "enforce":
            return (
                False,
                f"hook {rel} not enrolled in hooks-manifest.json — {_ENFORCE_HINT}",
            )
        return True, None
...
def verify_trusted_script(
    ...
    return (
        False,
        f"trusted-script sha256 mismatch (expected {expected[:12]}…, got {actual[:12]}…)",
    )
