...
def _declared_work_plane_contexts(
    project_root: Path,
) -> tuple[tuple[str, str, Path], ...]:
    specs_dir = _pointer_specs_dir(project_root) or (
        project_root / _AI_ENGINEERING_DIR / "specs"
    )
    return (
...
def _compute_prev_event_hash(path: Path) -> str | None:
    ...
    if not isinstance(prior, dict):
        return None
    stripped = {
        k: v for k, v in prior.items() if k not in ("prev_event_hash", "prevEventHash")
    }
...
def build_framework_event(
    ...
    * ``usage`` -- flat dict; reshaped into ``detail.genai`` when
    ...
    if trace_id is None and parent_span_id is None:
        resolved_trace_id, resolved_parent_span_id = _tc.current_trace_context(
            project_root
        )
