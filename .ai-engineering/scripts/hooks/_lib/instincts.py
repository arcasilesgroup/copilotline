...
def _load_yaml_or_json(path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        print(
            f"[instincts] _load_yaml_or_json IO error on {path}: {exc}", file=sys.stderr
        )
        return {}
    if not raw:
        return {}
    try:
        parsed = (
            yaml.safe_load(raw) or {} if _HAS_YAML else (json.loads(raw) if raw else {})
        )
    except Exception as exc:  # yaml.YAMLError + json.JSONDecodeError + anything weird; best-effort by contract
        print(
            f"[instincts] _load_yaml_or_json parse error on {path}: {exc}",
            file=sys.stderr,
        )
        return {}
...
def _sanitize_text(value: str | None) -> str | None:
    ...
    return redacted[:MAX_SUMMARY_LEN] + (
        "..." if len(redacted) > MAX_SUMMARY_LEN else ""
    )
...
def _build_observation_detail(
    data: dict[str, Any], *, hook_event: str
) -> dict[str, Any]:
    ...
    detail: dict[str, Any] = {
        ...
        "output_summary": _sanitize_text(
            _coerce_text(tool_output or data.get("error"))
        ),
...
def _filter_new_observations(
    ...
    if last_extracted_at is None:
        return observations
    return [
        entry for entry in observations if _obs_timestamp(entry) > last_extracted_at
    ]
...
def apply_confidence_decay(
    ...
    for entry in entries:
        ...
        if weeks_inactive > 0:
            current = entry.get(
                "confidence", confidence_for_count(entry.get("evidenceCount", 1))
            )
...
def _detect_error_recoveries(
    ...
    for entries in sessions.values():
        for current, nxt in pairwise(entries):
            if (
                current.get("kind") != "tool_complete"
                or current.get("outcome") != "failure"
            ):
                continue
