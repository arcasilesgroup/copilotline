...
def _read_raw(project_root: Path) -> dict[str, Any] | None:
    ...
    if not isinstance(payload, dict):
        _emit_corruption_event(
            project_root,
            summary=f"risk-score.json wrong type: {type(payload).__name__}",
        )
        return None
    return payload
...
def get(
    project_root: Path, *, session_id: str, now: datetime | None = None
) -> RiskState:
    ...
    * Matching session -> persisted score with decay applied since
    ...
    try:
        ...
        _emit_corruption_event(
            project_root, summary="risk-score.json non-numeric score"
        )
        return _make_fresh(session_id, now=reference)

    last_update = (
        payload.get("last_update_ts") or payload.get("lastUpdateTs") or _iso(reference)
    )
