...
``$HOME/.claude/projects/<project-slug>/<session-id>.jsonl``, where
``<project-slug>`` is the project path encoded per ``_project_slug`` below)
...
def _project_slug(project_root: Path) -> str:
    ...
    ``/home/user/repos/ai-engineering`` -> ``-home-user-repos-ai-engineering``.
    """
