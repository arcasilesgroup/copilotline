...
def _handle_pre_tool_use(data: dict, server_name: str, project_root: Path) -> None:
    ...
    if server["status"] == "healthy":
        ...
        if is_healthy:
            ...
            _save_state(state)
            _emit_health_change(
                project_root, server_name, old_status, "unhealthy", "probe failed"
            )
...
def _handle_post_tool_use_failure(
    data: dict, server_name: str, project_root: Path
) -> None:
    ...
    if old_status != "unhealthy":
        _emit_health_change(
            project_root, server_name, old_status, "unhealthy", error_str[:200]
        )
    ...
    if is_debug_mode():
        ...
        try:
            ...
            with open(debug_log, "a", encoding="utf-8") as f:
                f.write(
                    f"[{timestamp}] mcp-health: event={ctx.event_name} server={server_name}\n"
                )
        except Exception:
            pass
