...
def main() -> None:
    ...
    history = recent_tool_history(
        ctx.project_root, session_id=session_id, limit=LOOP_WINDOW
    )
    ...
    risk_disabled = (
        os.environ.get("AIENG_RISK_ACCUMULATOR_DISABLED") or ""
    ).strip() == "1"
    if not risk_disabled:
        try:
            from _lib import risk_accumulator  # type: ignore[import-not-found]

            state = risk_accumulator.get(
                ctx.project_root, session_id=session_id or "unknown"
            )
