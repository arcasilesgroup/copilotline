...
def main() -> None:
    ...
    project_root_env = os.environ.get("CLAUDE_PROJECT_DIR") or os.environ.get(
        "CODEX_PROJECT_DIR"
    )
    project_root = (
        Path(project_root_env) if project_root_env else hc._resolve_project_root()
    )
    ...
    if event_name in _HOOK_KIND_MAP:
        _emit_ide_hook(
            hc, project_root, event_name, payload, correlation_id, session_id
        )
        return
